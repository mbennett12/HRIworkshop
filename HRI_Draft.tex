...
\title{\LARGE \bf Differences in Interaction Patterns and Perception\\ for
  Teleoperated and Autonomous Humanoid Robots}
%
...
%
  ...
  %
  % 1st. author
  \author{Maxwell Bennett, Tom Williams, Daria Thames and Matthias Scheutz
  \thanks{Maxwell Bennett is now with Kindred (www.kindred.ai). Tom Williams,
    Daria Thames, and Matthias Scheutz are with the Human-Robot Interaction Laboratory in the
    Department of Computer Science, Tufts University, Medford MA
    02155, USA, where this work was performed. The authors can be
    reached at: {\small maxwellbbennett@gmail.com, 
      williams@cs.tufts.edu, daria.thames@tufts.edu, matthias.scheutz@tufts.edu}
  }
  }
...
\begin{abstract}
As the linguistic capabilities of interactive robots advance, %in
%sophistication, 
it becomes increasingly important to understand how
humans will instruct robots through natural language.
%% , and how
%% this might differ from how they instruct fellow humans.
What is
...
experiment in which participants interacted in a collaborative, 
task-based setting with both a human and a 
...
(Max \textit{slightly} modified)
As the physical capabilities of interactive robots advances,
it becomes increasingly important to understand how
...
task instructions. 
Thus, in this work we present an experiment
...
and has used robots controlled
through graphical interfaces.
In contrast, we study actual \textit{interactions}, and use a novel,
immersive interface in which teleoperator motions are replicated by a
robot in real time.
...
agents~\cite{oztop2005human}. \textbf{[@TW, is this next sentence still true?]}
Furthermore, to the best of our knowledge,
%@TW are we sure that *all* is true? all from our analysis, at least...
all previous
\emph{empirical research} involving teleoperated robots has relied on
...
control. This 
 ...
 interaction~\cite{williams2017chi,mutlu2009footing,Kindred}
...
\section{Methodology}
\label{sec:exp}
We will now describe the details of our hypotheses, experimental design, procedure, and measurements.
...
\cite{cha2015perceived,kim2006should}, we hypothesized \textbf{(H3)}
...
which each
participant was required to %% sequentially
teach a new skill to a \textbf{human} learner and to a \textbf{robot}
learner.
...
complete a task in a large experiment
...
miniature (Figs.~\ref{fig:roomsetup}~and~\ref{fig:diorama}). Both the diorama and experiment area were divided into four 
quadrants containing a variety of objects: four cardboard boxes
(each of which was labeled with a different letter),
and three colored towers (comprised of Lego blocks in the diorama and
aluminum cans in the experiment area). 

Participants were told to arrange their diorama \emph{however they
  wished}, with only a few limitations (i.e., cans could not be
translated, boxes could not be flipped over or stacked), %% (a crucial decision made to reduce the potential for
%% experimental bias),
...
replicate the arrangement of their diorama, and the learner carried
out the instructions to the best of their ability. Both human and
robot learners restricted their utterances to ``Okay'', ``Yes'', and
``No'' whenever possible, but also asked ``Are there any more
instructions?'' if it was not clear whether the interaction was
over. The robot's utterances were selected by a human confederate,
and synthesized using a text-to-speech interface.  
Once the interaction was finished, the agent said ``Goodbye'' and left
the room.  
...
2 DoF head. Encompassed in the robot's torso was a small
computer and WiFi 
adapter for wireless communication, with electronics and a battery in its
base, able to power the robot for a couple hours untethered.
...
Lastly, in order to standardize language and voice used throughout
...
gather data regarding participants' perceptions of their human and
robot teammates. 
Participants' views on the
following properties of the robot and human were analyzed:
cooperativity, capability, annoyingness, creepiness, responsiveness, 
...
\textit{complexity} of participants' arrangements, as assessed by the
...
%
%
%Questionnaire responses were analyzed using mixed ANOVAs with two
%independent variables: main experimental condition (autonomous
%vs. teleoperated, between-subjects), and interaction order (robot
%first vs. human-first, within-subjects). Participants' views on the
%following properties were analyzed: (list items we asked about)
%(1=strongly disagree to 7=strongly agree). Interaction effects were
%found for XYZ, as seen in table XYZ. For ABC, no effect of order was
%observed, and one-way ANOVAs assessing only the main experimental
%condition were used to better investigate the effects of the main
%condition. Significant effects of main condition were thus found for
%ABC, as seen in table ABC.
%
%An analysis of covariance (ANCOVA) was performed for the behavioral
%measures, using the same independent variables as the original
%analysis, and arrangement complexity as a covariate. The results
%showed that XYZ.
%
Participants' questionnaire responses were analyzed using mixed Analyses of Variance (ANOVAs) with two
independent variables: main experimental condition (autonomous \textbf{(AC)}
vs. teleoperated \textbf{(TC)}, between subjects), and interaction order (robot
first vs. human-first, between-subjects). 

For each of these variables, we first established that there were no
main or interaction effects involving order of interaction, and then
used ANOVAs to examine whether there were significant
differences between robot and human learners, or between teleoperated and
autonomous robot learners.

\subsection{Objective Measures}

While no differences were found in the percentage of learner-directed
directives that were phrased indirectly vs. directly, a factorial
logistic regression analysis yielded a marginal
interaction effect with respect to whether or not
participants used indirect utterances at all (odds ratio=0.13, probability value(p)=.053): In \textbf{AC}, 70.59\% of participants
used at least one ISA when directing another human but only 29.41\% of
participants used one when directing a robot; in \textbf{TC}, 50.0\% of participants used at least one ISA when speaking with another human, but 56.25\% of participants used
at least one when speaking to the teleoperated robot.

%% A significant difference was found, however, between the AUTONOMOUS and
%% TELEOPERATED conditions, for whether participants combined all their
%% instructions into a single command or whether participants broke their
%% instructions into individual commands that were delivered one at a
%% time. Specifically, a significant difference was found between
%% human-directed (in which instructions were provided in a single
%% command 49.15\% of the time (SD=28.24)) and robot-directed utterances
%% (in which instructions were provided in a single command 62.88\% of
%% the time (SD=28.41)), F(1,29)=7.62, p=.010. 

A significant difference was also found with respect to number of
words used to describe the task to robot learners between \textbf{AC}
and \textbf{TC}: %, %% when participants were told
%% that the robot was AUTONOMOUS vs. TELEOPERATED
%as seen in
%Fig.~\ref{fig:wca}: % The results suggest that 
participants used significantly more words to describe the task in the
\textbf{TC} (Mean(M)=204.88, Standard Deviation(SD)=181.60) than in \textbf{AC} (M=72.82, SD=42.83), F(1,29)=8.05, p=.008. An Analysis of
Covariance (ANCOVA) using task complexity as a covariate 
...
%% \end{figure}
%
Finally, a significant difference was found with respect to the
...
%
The results of our analyses suggest that participants in \textbf{AC}
...
purportedly autonomous robot (Fig.~\ref{fig:trans1}):
\begin{figure}[ht]
\begin{center}
...
We hypothesized \textbf{(H2)} that not only would robots described to
...
instructions and less likely to understand high level commands such as
``Arrange the blocks \textit{like this}' (C3). This is striking, as such
...
differently in this respect than they did the purportedly autonomous
robot. Furthermore, both autonomous \textit{and teleoperated} robots 
...
human) teleoperator, but also that the participant's use
of this language then carried over into his second interaction, now with a
...
higher levels of blame and scrutiny, we hypothesized \textbf{(H3)}
...
such strategies.
%
Our results also suggest variations in how these three types of
teammates were perceived. Specifically, our results suggest that human-teleoperated robots were perceived as less intelligent than human
teammates; a finding 
with serious implications for human-robot team dynamics.
